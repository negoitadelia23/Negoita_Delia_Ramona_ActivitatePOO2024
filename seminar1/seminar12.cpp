...
public:
	...
	friend ifstream& operator>>(ifstream& input, Masina m)
	{
		...
		input >> m.pret;
		int an;
		input >> an;//e auxiliara, nu face nimic
		return input;
	}
	void scriereFisiereBinar(fstream& fisier) {
		fisier.write((char*)&nrRoti, sizeof(int));
		for (int i = 0; i < nrRoti; i++)
		{
			int lungime = producatori[i].length();
			fisier.write((char*)&lungime, sizeof(int));
			fisier.write((char*)(producatori[i].c_str()), sizeof(char) * lungime);
		}
...
public:
	...
	Vector(int nr, T** elem)
	{
		this->nrElem = nr;
		this->elemente = new T * [nr];
		for (int i = 0; i < nr; i++)
			this->elem[i] = new T(*elem[i]);
	}
	...
	void adaugaElement(T* elemNou)
	{
		T** aux = new T * [this->nrElem + 1];
	...
	float calculeazaTotal()//la template nu trb sa precizeze CE total calculez, trb schimbat in functie de ce vreau
	{
		float sum = 0;
		for (int i = 0; i < nrElem; i++)
			sum += (float)*this->elem[i];
		return sum;
	}
	friend ostream& operator<<(ostream& out, Vector& v)
	{
		out << "Nr de elemente: " << v.nrElem << endl;
		for (int i = 0; i < v.nrElem; i++)
			out << "Elementul " << i + 1 << ": " << *v.elem[i] << endl;
		return out;
	}
};
int main() {
